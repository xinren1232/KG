...
# -*- coding: utf-8 -*-
"""
终极合并所有补充数据生成完整的导入脚本(包含16个硬件模块)
"""

import json
from datetime import datetime

def ultimate_merge_all_16_modules_data():
    """终极合并所有补充数据(包含16个硬件模块)"""
    
    print("🚀 开始终极合并所有补充数据(包含16个硬件模块)...")
    
    # 读取现有的Cypher脚本
    scripts_to_merge = [
        ('补充数据导入脚本.cypher', '基础补充数据', 139),
        ('全部16个硬件模块数据导入脚本.cypher', '16个硬件模块数据', 401)
    ]
    ...
    # 生成终极完整的合并脚本
    with open('终极完整词典补充数据导入脚本_16模块版.cypher', 'w', encoding='utf-8') as f:
        f.write("// 终极完整词典补充数据导入脚本(16个硬件模块版)\n")
        f.write(f"// 生成时间: {datetime.now().isoformat()}\n")
        f.write(f"// 总计: {total_statements}条补充数据\n")
        f.write("// 包含: 基础补充数据(139条) + 16个硬件模块数据(401条)\n")
        f.write("//\n")
        f.write("// 数据构成:\n")
        ...
        f.write("//   - 连接技术: Wi-Fi、蓝牙、5G、GNSS、NFC\n")
        f.write("//   - 接口技术: USB-C、Lightning、FPC、BTB\n")
        f.write("//   - 生产技术: ICT、FCT、自动化测试、治具设计\n")
        f.write("//\n")
        f.write("// Label分布:\n")
        for label, count in sorted(label_counts.items(), key=lambda x: x[1], reverse=True):
    ...
    print(f"\n✅ 终极完整合并脚本已生成: 终极完整词典补充数据导入脚本_16模块版.cypher")
    ...
    ultimate_report = {
        ...
        'files_generated': ['终极完整词典补充数据导入脚本_16模块版.cypher'],
        'system_enhancement': {
            'original_data': 526,
            'basic_supplement': script_stats.get('基础补充数据', 0),
            'hardware_modules': script_stats.get('16个硬件模块数据', 0),
        ...
        'hardware_coverage': {
            ...
            '第四批模块': '连接网络(30)、接口连接器(24)、被动元件(28)、生产测试治具(20)',
            '技术领域': '14大硬件技术领域完整覆盖',
        ...
        'tech_domains': {
            ...
            '连接技术': '30条 - Wi-Fi、蓝牙、5G、GNSS、NFC',
            '接口技术': '24条 - USB-C、Lightning、FPC、BTB',
            '生产技术': '20条 - ICT、FCT、自动化测试、治具设计'
        },
        'quality_metrics': {
            ...
            'module_coverage': '16个硬件模块系统性覆盖',
            'tech_breadth': '14大技术领域全面覆盖'
        }
    }
    
    with open('终极完整补充数据合并报告_16模块版.json', 'w', encoding='utf-8') as f:
        json.dump(ultimate_report, f, ensure_ascii=False, indent=2)
    
    print(f"✅ 终极合并报告已生成: 终极完整补充数据合并报告_16模块版.json")
    
    return total_statements, label_counts, script_stats, ultimate_report

def main():
    print("📋 终极词典补充数据合并处理(16个硬件模块版)...")
    
    total, labels, sources, report = ultimate_merge_all_16_modules_data()
    ...
    if total > 0:
        ...
        print(f"2. 执行终极完整补充数据导入脚本(16模块版)")
