...
def merge_all_hardware_modules():
    """合并全部硬件模块数据"""
    ...
    all_data = []
    module_stats = {}
    batch_stats = {'第一批': 0, '第二批': 0}
    
    print("🚀 开始合并全部8个硬件模块词典数据...")
    ...
    if all_data:
        ...
        # 保存Cypher脚本
        with open('全部硬件模块数据导入脚本.cypher', 'w', encoding='utf-8') as f:
            f.write("// 全部硬件模块词典数据导入脚本\n")
            f.write(f"// 生成时间: {datetime.now().isoformat()}\n")
            f.write(f"// 总计: {len(cypher_statements)}条硬件模块数据\n")
            f.write("// 包含8大硬件模块: 显示屏、摄像头、电池、主板PCBA、射频天线、声学、结构连接器、散热系统\n")
            f.write("//\n")
            f.write("// 技术领域覆盖:\n")
            for domain, modules in tech_domains.items():
                domain_total = sum(module_stats.get(module, {}).get('total', 0) for module in modules)
        ...
        print(f"\n✅ 完整硬件模块Cypher脚本已生成: 全部硬件模块数据导入脚本.cypher")
        ...
        complete_report = {
            ...
            'files_generated': ['全部硬件模块数据导入脚本.cypher'],
            'module_coverage': {
        ...
        with open('全部硬件模块数据统计报告.json', 'w', encoding='utf-8') as f:
            json.dump(complete_report, f, ensure_ascii=False, indent=2)
        
        print(f"✅ 完整统计报告已生成: 全部硬件模块数据统计报告.json")
...
def main():
    print("📋 全部硬件模块词典数据合并处理...")
    
    total, labels, modules, domains = merge_all_hardware_modules()
    
    if total > 0:
        print(f"\n📈 合并完成!")
        print(f"总计合并: {total}条硬件模块数据")
        print(f"涵盖模块: 8个核心硬件模块")
        print(f"技术领域: 8大硬件技术领域")
